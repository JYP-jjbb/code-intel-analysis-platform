...
@RestController
@RequestMapping("/api/settings/hunyuan-key")
@Validated
public class HunyuanSettingsController {
    private static final Logger log = LoggerFactory.getLogger(HunyuanSettingsController.class);

    private final SiliconFlowSettingsService settingsService;

    public HunyuanSettingsController(SiliconFlowSettingsService settingsService) {
        this.settingsService = settingsService;
    }

    @GetMapping
    public SiliconFlowApiKeyResponse getKeyStatus() {
        SiliconFlowSettingsService.ApiKeyStatus status = settingsService.getHunyuanApiKeyStatus();
        log.info("Hunyuan key status queried: hasApiKey={}, masked={}, target={}",
                status.isHasApiKey(),
    ...
    public SiliconFlowApiKeyResponse saveApiKey(@Valid @RequestBody SiliconFlowApiKeyRequest request) {
        try {
            SiliconFlowSettingsService.ApiKeyStatus status = settingsService.saveHunyuanApiKey(request.getApiKey());
            log.info("Hunyuan key saved: hasApiKey={}, masked={}, updatedAt={}",
                    status.isHasApiKey(),
                    status.getMaskedApiKey(),
                    status.getUpdatedAt());
            return toResponse(status);
