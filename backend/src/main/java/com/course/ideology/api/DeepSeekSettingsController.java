...
@RestController
@RequestMapping("/api/settings/deepseek-key")
@Validated
public class DeepSeekSettingsController {
    private static final Logger log = LoggerFactory.getLogger(DeepSeekSettingsController.class);

    private final SiliconFlowSettingsService settingsService;

    public DeepSeekSettingsController(SiliconFlowSettingsService settingsService) {
        this.settingsService = settingsService;
    }

    @GetMapping
    public SiliconFlowApiKeyResponse getKeyStatus() {
        SiliconFlowSettingsService.ApiKeyStatus status = settingsService.getDeepSeekApiKeyStatus();
        log.info("DeepSeek key status queried: hasApiKey={}, masked={}, target={}",
                status.isHasApiKey(),
    ...
    public SiliconFlowApiKeyResponse saveApiKey(@Valid @RequestBody SiliconFlowApiKeyRequest request) {
        try {
            SiliconFlowSettingsService.ApiKeyStatus status = settingsService.saveDeepSeekApiKey(request.getApiKey());
            log.info("DeepSeek key saved: hasApiKey={}, masked={}, updatedAt={}",
                    status.isHasApiKey(),
                    status.getMaskedApiKey(),
                    status.getUpdatedAt());
            return toResponse(status);
